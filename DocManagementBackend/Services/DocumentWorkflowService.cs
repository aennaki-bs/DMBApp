        ...
        public async Task<List<StatusDto>> GetAvailableTransitionsAsync(int documentId)
        {
            ...
            var statuses = await _context.Status
                ...
                .Select(s => new StatusDto
                {
                    ...
                    // Include steps that led to this status for UI visualization
                    TransitionInfo = steps
                        .Where(step => step.NextStatusId == s.Id)
                        .Select(step => step.Title)
                        .FirstOrDefault() ?? $"Move to {s.Title}"
                })
                .ToListAsync();
            ...
            var flexibleStatuses = await _context.Status
                ...
                .Select(s => new StatusDto
                {
                    ...
                    CircuitId = s.CircuitId,
                    TransitionInfo = $"Move to {s.Title} (Flexible Status)"
                })
                .ToListAsync();
