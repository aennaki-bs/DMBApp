...
namespace DocManagementBackend.Services
{
    public class DocumentWorkflowService
    {
        ...
        // ProcessActionAsync method removed - Actions functionality has been removed from the system
        ...
        public async Task<bool> MoveToNextStatusAsync(int documentId, int targetStatusId, int userId, string comments)
        {
            ...
            if (targetStatus.IsFinal)
            {
                ...
                // Trigger ERP archival synchronously (safer than async Task.Run for DI scope)
                _logger.LogInformation("Document {DocumentId} reached final status '{StatusTitle}'. Starting ERP archival process.", documentId, targetStatus.Title);
            ...
            int documentId, int statusId, int userId, bool isComplete, string comments)
        {
            ...
            if (isComplete)
            {
                ...
                if (documentStatus.Status?.IsFinal == true)
                {
                    ...
                    // Trigger ERP archival synchronously (safer than async Task.Run for DI scope)
                    _logger.LogInformation("Document {DocumentId} final status completed. Starting ERP archival process.", documentId);
