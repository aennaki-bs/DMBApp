...
import { ElementTypesTableHeader } from "./content/ElementTypesTableHeader";
import { ElementTypesTableBody } from "./content/ElementTypesTableBody";
import { ElementTypesTableEmpty } from "./ElementTypesTableEmpty";
import { ScrollArea } from "@/components/ui/scroll-area";
import PaginationWithBulkActions, { BulkAction } from "@/components/shared/PaginationWithBulkActions";
import { Loader2, Trash2 } from "lucide-react";

interface ElementTypesTableContentProps {
    elementTypes: LignesElementType[] | undefined;
    allElementTypes?: LignesElementType[] | undefined;
    selectedElementTypes: any[];
    bulkSelection: any; // Using any to match UserTableContent pattern
    pagination: {
        currentPage: number;
        pageSize: number;
        totalPages: number;
...
export function ElementTypesTableContent({
    elementTypes,
    allElementTypes,
    ...
    // Check if we have element types to display
    const hasElementTypes = elementTypes && elementTypes.length > 0;

    // Define bulk actions
    const bulkActions: BulkAction[] = [
        {
            id: 'delete',
            label: 'Delete Element Types',
                    ...
                    <div className="flex items-center justify-center py-20">
                        <div className="flex flex-col items-center gap-4">
                            ...
                            <p className="text-destructive">
                                Failed to load element types. Please try again.
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div className="flex-1 relative overflow-hidden rounded-xl border border-primary/10 bg-gradient-to-br from-background/80 via-background/60 to-background/80 backdrop-blur-xl shadow-lg min-h-0">
                ...
                {hasElementTypes ? (
                            ...
                            <div className="min-w-[1026px]">
                                <Table className="table-fixed w-full">
                                    <ElementTypesTableHeader
                                        ...
                                        totalCount={elementTypes?.length || 0}
                                ...
                                <div className="min-w-[1026px] pb-4">
                                    <Table className="table-fixed w-full">
                                        <ElementTypesTableBody
                                            ...
                                            onSelectElementType={(id) => {
                                                const elementType = elementTypes?.find(et => et.id === id);
                                                if (elementType) bulkSelection.toggleItem(elementType);
                                            }}
                                            onEdit={onEdit}
                                            onView={onView}
                                            onDelete={onDelete}
                    ...
                    <div className="relative h-full flex items-center justify-center z-10">
                        <ElementTypesTableEmpty onClearFilters={onClearFilters} />
                    </div>
                )}
            </div>
