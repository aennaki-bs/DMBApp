...
import { Item, GeneralAccounts, CreateLignesElementTypeRequest } from '@/models/lineElements';
...
const step2Schema = z.object({
  tableName: z.string().min(1, 'Table name is required').max(100, 'Table name must be 100 characters or less'),
  description: z.string().max(255, 'Description must be 255 characters or less').optional().transform((val) => val?.trim() || ''),
});
...
const CreateElementTypeWizard = ({ 
  ...
  }, [isOpen, step1Form, step2Form, step3Form, step4Form]);
  ...
  const validateCode = async (code: string) => {
    ...
    try {
      ...
      const codeExists = existingTypes.some(type => 
  ...
  const handleSubmit = async () => {
    if (!wizardData.code || !wizardData.tableName || !wizardData.typeElement || !wizardData.selectedElementCode) {
    ...
    try {
      const createRequest: CreateLignesElementTypeRequest = {
        ...
        itemCode: wizardData.typeElement === 'Item' ? wizardData.selectedElementCode : undefined,
        accountCode: wizardData.typeElement === 'General Accounts' ? wizardData.selectedElementCode : undefined,
      };
  ...
  const getSelectedElementDetails = () => {
    ...
    if (wizardData.typeElement === 'Item') {
      return items.find(item => item.code === wizardData.selectedElementCode);
    } else {
      return generalAccounts.find(account => account.code === wizardData.selectedElementCode);
    }
  };
      ...
      <DialogContent className="bg-[#0a1033] border-blue-900/30 text-white max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription className="text-blue-300">
            Follow the steps to create a new line element type with proper associations
          </DialogDescription>
        </DialogHeader>
            ...
            <Card className="bg-[#0f1642] border-blue-900/30">
              <CardHeader>
                ...
                <CardDescription className="text-blue-300">
                  Enter a unique code for the element type. This code will be used to identify the element type.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <Form {...step1Form}>
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel className="text-blue-300">Element Type Code *</FormLabel>
                        <FormControl>
                          <div className="relative">
                            ...
                            <div className="absolute right-3 top-1/2 transform -translate-y-1/2">
                              ...
                              {!isValidatingCode && !isCodeValid && field.value && (
                ...
                <div className="bg-blue-900/20 border border-blue-700/30 rounded-lg p-4">
                  <h4 className="text-sm font-medium text-blue-300 mb-2">Code Guidelines:</h4>
          ...
          {currentStep === 2 && (
            <Card className="bg-[#0f1642] border-blue-900/30">
              <CardHeader>
                ...
                <CardDescription className="text-blue-300">
                  Provide the table name and an optional description for the element type.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <Form {...step2Form}>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel className="text-blue-300">Description (Optional)</FormLabel>
                        <FormControl>
                          <Input
                ...
                <div className="bg-amber-900/20 border border-amber-700/30 rounded-lg p-4">
                  <h4 className="text-sm font-medium text-amber-300 mb-2">Information:</h4>
                  <ul className="text-xs text-amber-400 space-y-1">
                    <li>• Table name should match the database table or entity</li>
                    <li>• Description is optional - if left empty, a description will be auto-generated</li>
                    <li>• Auto-generated format: "[Type Element] element type for [Table Name]"</li>
                  </ul>
                </div>
              </CardContent>
            </Card>
          ...
          {currentStep === 3 && (
            <Card className="bg-[#0f1642] border-blue-900/30">
              <CardHeader>
                ...
                <CardDescription className="text-blue-300">
                  Choose whether this element type will reference Items or General Accounts.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <Form {...step3Form}>
                            ...
                            >
                              <CardContent className="p-4 text-center">
                                <Package className={`h-8 w-8 mx-auto mb-2 ${
                                  field.value === 'Item' ? 'text-emerald-400' : 'text-blue-400'
                                }`} />
                            ...
                            >
                              <CardContent className="p-4 text-center">
                                <Calculator className={`h-8 w-8 mx-auto mb-2 ${
                                  field.value === 'General Accounts' ? 'text-violet-400' : 'text-blue-400'
                                }`} />
                                <h3 className="font-medium text-white">General Accounts</h3>
                                <p className="text-xs text-blue-300 mt-1">
          ...
          {currentStep === 4 && (
            <Card className="bg-[#0f1642] border-blue-900/30">
              <CardHeader>
                ...
                <CardDescription className="text-blue-300">
                  Choose the specific {wizardData.typeElement?.toLowerCase()} that this element type will reference.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <Form {...step4Form}>
                        ...
                        <FormControl>
                          <Select onValueChange={field.onChange} value={field.value}>
                            <SelectTrigger className="bg-[#111633] border-blue-900/50 text-white">
                              <SelectValue placeholder={`Select a ${wizardData.typeElement?.toLowerCase()}`} />
                            </SelectTrigger>
                                ...
                                generalAccounts.map((account) => (
                                  <SelectItem key={account.code} value={account.code}>
                                    <div className="flex items-center">
                                      ...
                                      <span>{account.code} - {account.description}</span>
                                    </div>
                                  </SelectItem>
                                ))
                              )}
                ...
                {wizardData.typeElement && (
                  <div className="bg-blue-900/20 border border-blue-700/30 rounded-lg p-4">
                    <h4 className="text-sm font-medium text-blue-300 mb-2">Available {wizardData.typeElement}:</h4>
                    <p className="text-xs text-blue-400">
                      {wizardData.typeElement === 'Item' 
                        ? `${items.length} items available`
                        : `${generalAccounts.length} general accounts available`
                      }
                    </p>
                  </div>
                )}
            ...
            <Card className="bg-[#0f1642] border-blue-900/30">
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-3">
                    <div>
                      ...
                      <div className="bg-[#111633] border border-blue-900/50 rounded-md p-2">
                        <p className="text-white font-mono">{wizardData.code}</p>
                      </div>
                    </div>

                    <div>
                      <Label className="text-blue-300 text-sm">Table Name</Label>
                      <div className="bg-[#111633] border border-blue-900/50 rounded-md p-2">
                      ...
                      <div className="bg-[#111633] border border-blue-900/50 rounded-md p-2">
                        ...
                        {(!wizardData.description || !wizardData.description.trim()) && (
                          <p className="text-xs text-amber-400 mt-1">
                            * Auto-generated description (no custom description provided)
                          </p>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="space-y-3">
                    <div>
                      <Label className="text-blue-300 text-sm">Type Element</Label>
                      <div className="bg-[#111633] border border-blue-900/50 rounded-md p-2">
                        <Badge className={`${wizardData.typeElement === 'Item' 
                    ...
                    <div>
                      <Label className="text-blue-300 text-sm">Selected Element</Label>
                      <div className="bg-[#111633] border border-blue-900/50 rounded-md p-2">
                        {(() => {
                          const element = getSelectedElementDetails();
                          return element ? (
                            <div>
                              <p className="text-white font-medium">{element.code}</p>
                              <p className="text-blue-300 text-sm">{element.description}</p>
                            </div>
                          ) : (
                ...
                <div className="bg-green-900/20 border border-green-700/30 rounded-lg p-4">
                  ...
                  <p className="text-xs text-green-400">
                    All required information has been provided. Click "Create Element Type" to submit.
                  </p>
                </div>
              </CardContent>
            </Card>
