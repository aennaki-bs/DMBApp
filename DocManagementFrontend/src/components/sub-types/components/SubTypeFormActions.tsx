...
export const SubTypeFormActions = ({ onCancel }: SubTypeFormActionsProps) => {
  const { currentStep, nextStep, prevStep, handleSubmit, validateCurrentStep, isLoading } = useSubTypeForm();
      ...
      <div>
        {currentStep < 3 ? (
          ...
          <Button
            ...
            disabled={isLoading}
            className="h-10 bg-blue-600 hover:bg-blue-700 text-white font-medium px-4"
          >
            {isLoading ? (
              <Loader2 className="h-4 w-4 mr-2 animate-spin" />
            ) : (
