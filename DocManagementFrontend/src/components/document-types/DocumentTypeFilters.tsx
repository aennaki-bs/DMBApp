...
const DocumentTypeFilters = ({ onFilterChange, onClose }: DocumentTypeFiltersProps) => {
  ...
  const handleApplyFilters = () => {
    ...
    if ((filterType !== 'createdAt' && filterType !== 'updatedAt' && filterValue) || 
        ((filterType === 'createdAt' || filterType === 'updatedAt') && dateRange?.from)) {
      
      const newFilter = {
        id: Date.now(),
        type: filterType,
        value: filterType === 'createdAt' || filterType === 'updatedAt' 
          ? dateRange 
          : filterValue,
        display: filterType === 'createdAt' || filterType === 'updatedAt'
  ...
  const getFilterLabel = (type: string) => {
    switch(type) {
      ...
      case 'typeKey': return 'Type Code';
      case 'documentCounter': return 'Document Count';
  ...
  return (
    <Card className="bg-[#0a1033] border border-blue-900/30 rounded-md overflow-hidden">
      <div className="px-4 py-3 bg-blue-900/20 flex justify-between items-center">
        ...
        <Button variant="ghost" size="sm" onClick={onClose} className="h-8 w-8 p-0 hover:bg-blue-800/50">
          <X className="h-4 w-4" />
        </Button>
      </div>
      ...
      <CardContent className="p-4 grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          {(filterType === 'createdAt' || filterType === 'updatedAt') ? (
            ...
            <div>
              ...
              <div className="relative w-full">
                ...
                <Input 
                  placeholder={`Enter ${getFilterLabel(filterType).toLowerCase()}...`}
                  value={filterValue}
        ...
        <div className="md:col-span-1 flex items-end">
          <Button 
            ...
            disabled={(filterType !== 'createdAt' && filterType !== 'updatedAt' && !filterValue) || 
                     ((filterType === 'createdAt' || filterType === 'updatedAt') && !dateRange?.from)}
          >
            Add Filter
          </Button>
        ...
        <>
          ...
          <div className="p-4">
            <div className="flex justify-between items-center mb-2">
              <h4 className="text-xs font-medium text-blue-300">Active Filters</h4>
              <Button 
                variant="ghost" 
                size="sm"
            ...
            <div className="flex flex-wrap gap-2">
              {activeFilters.map(filter => (
                ...
                >
                  <span className="text-xs font-medium text-blue-400">{getFilterLabel(filter.type)}:</span>
