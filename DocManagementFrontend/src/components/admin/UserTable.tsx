...
export function UserTable() {
  ...
  const handleToggleUserStatus = async (
    ...
    try {
      ...
      toast.success(newStatus ? t("userManagement.userActivated") : t("userManagement.userBlocked"));
      refetch();
    } catch (error) {
      toast.error(currentStatus ? t("userManagement.failedToBlock") : t("userManagement.failedToActivate"));
      console.error(error);
    }
  };
  ...
  const handleUserRoleChange = async (userId: number, roleName: string) => {
    try {
      ...
      toast.success(tWithParams("userManagement.roleChanged", { role: roleName }));
      refetch();
    } catch (error) {
  ...
  const handleBulkRoleChange = async () => {
    ...
    try {
      ...
      toast.success(
        tWithParams("userManagement.roleUpdateSuccess", { role: selectedRole, count: selectedUsers.length })
      );
      refetch();
  ...
  const handleDeleteMultiple = async () => {
    try {
      ...
      toast.success(tWithParams("userManagement.deleteSuccess", { count: selectedUsers.length }));
      handleMultipleDeleted();
    } catch (error) {
  ...
  const roleOptions = [
    ...
    { id: "SimpleUser", label: t("userManagement.simpleUser"), value: "SimpleUser" },
  ];
          ...
          <PopoverContent className="w-80 bg-[#1e2a4a] border border-blue-900/40 rounded-xl shadow-lg p-4 animate-fade-in">
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-1">
                <span className="text-sm text-blue-200">{t("userManagement.status")}</span>
              ...
              {/* Role Filter */}
              <div className="flex flex-col gap-1">
                <span className="text-sm text-blue-200">{t("userManagement.role")}</span>
