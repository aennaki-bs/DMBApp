...
import { ItemsTableEmpty } from "./table/ItemsTableEmpty";
...
export const ItemsTable: React.FC = () => {
  ...
  const filteredAndSortedItems = useMemo(() => {
    let filtered = items.filter((item) => {
      ...
      switch (searchField) {
        ...
        case "unite":
          return (item.unite || "").toLowerCase().includes(query);
        default:
          return (
            item.code.toLowerCase().includes(query) ||
            item.description.toLowerCase().includes(query) ||
            (item.unite || "").toLowerCase().includes(query)
          );
      }
    });
    ...
    return filtered;
  }, [items, searchQuery, searchField, sortField, sortDirection]);
  ...
  const handleDeleteItem = (item: Item) => {
    toast.info("Delete item functionality coming soon");
  };

  const handleViewItem = (item: Item) => {
    toast.info("View item functionality coming soon");
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {filteredAndSortedItems.length === 0 ? (
        <ItemsTableEmpty hasSearchQuery={!!searchQuery} onClearSearch={clearSearch} />
      ) : (
        <ItemsTableContent
