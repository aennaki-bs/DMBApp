...
import { useUnitCodeManagement } from "@/hooks/useUnitCodeManagement";
import { DEFAULT_UNIT_CODE_SEARCH_FIELDS } from "@/components/table/constants/filters";

export function UnitCodesTable() {
  const {
    // Data
    unitCodes: filteredUnitCodes,
    paginatedUnitCodes,
    isLoading,
    isError,
    refetch,
    ...
    clearAllFilters,
  } = useUnitCodeManagement();

  // Handle unit code actions (disabled)
  const handleEditUnitCode = (unitCode: any) => {
    // Disabled - no action
  };

  const handleViewUnitCode = (unitCode: any) => {
    // Disabled - no action
  };

  const handleDeleteUnitCode = (code: string) => {
    // Disabled - no action
  };
  ...
  // Show filter only if there are items to filter or active filters
  const showFilter = hasActiveFilters || (filteredUnitCodes && filteredUnitCodes.length > 0);
    ...
    >
          ...
          <div className="relative">
            <Select value={searchField} onValueChange={setSearchField}>
              <SelectTrigger className="w-[140px] h-12 bg-background/60 backdrop-blur-md text-foreground border border-primary/20 hover:border-primary/40 focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all duration-300 hover:bg-background/80 shadow-lg rounded-xl">
                <SelectValue>
                  {DEFAULT_UNIT_CODE_SEARCH_FIELDS.find(
              ...
              <SelectContent className="bg-background/95 backdrop-blur-xl text-foreground border border-primary/20 rounded-xl shadow-2xl">
                {DEFAULT_UNIT_CODE_SEARCH_FIELDS.map((opt) => (
                  <SelectItem
                    key={opt.id}
                    ...
                    <div>
                      ...
                      <Select value={searchField} onValueChange={setSearchField}>
                        ...
                        <SelectContent className="bg-background/95 backdrop-blur-md border-primary/20">
                          {DEFAULT_UNIT_CODE_SEARCH_FIELDS.map((option) => (
      ...
      {/* <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Badge variant="secondary" className="bg-primary/10 text-primary border-primary/20">
            ...
            {filteredUnitCodes?.length || 0} unit codes
          </Badge>
          {searchQuery && (
      ...
      <div className="flex-1 min-h-0">
        <UnitCodesTableContent
          ...
          allUnitCodes={filteredUnitCodes}
          selectedItems={selectedItems}
          bulkSelection={bulkSelection}
          pagination={pagination}
          onEdit={handleEditUnitCode}
          onView={handleViewUnitCode}
