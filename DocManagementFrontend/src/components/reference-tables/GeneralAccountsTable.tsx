...
import { Calculator, Search, Filter, X, Plus } from "lucide-react";
...
import { GeneralAccountsTableContent } from "./table/GeneralAccountsTableContent";
import { GeneralAccountsTableEmpty } from "./table/GeneralAccountsTableEmpty";
...
export const GeneralAccountsTable: React.FC = () => {
  // Data fetching
  const {
    data: accounts = [],
    isLoading,
  ...
  const filteredAndSortedAccounts = useMemo(() => {
    let filtered = accounts.filter((account) => {
      ...
      switch (searchField) {
        ...
        case "description":
          return account.description.toLowerCase().includes(query);
        case "accountType":
          return (account.accountType || "").toLowerCase().includes(query);
        default:
          return (
    ...
    return filtered;
  }, [accounts, searchQuery, searchField, sortField, sortDirection]);
  ...
  const handleEditAccount = (account: GeneralAccounts) => {
    // TODO: Implement edit functionality
    toast.info("Edit account functionality coming soon");
  };

  const handleDeleteAccount = (account: GeneralAccounts) => {
    // TODO: Implement delete functionality
    toast.info("Delete account functionality coming soon");
  };

  const handleViewAccount = (account: GeneralAccounts) => {
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="flex flex-col items-center space-y-4">
          ...
          <p className="text-primary font-medium">Loading accounts...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {filteredAndSortedAccounts.length === 0 ? (
        <GeneralAccountsTableEmpty hasSearchQuery={!!searchQuery} onClearSearch={clearSearch} />
      ) : (
        <GeneralAccountsTableContent
            ...
            >
              <div className="bg-gradient-to-r from-primary/95 to-primary/90 backdrop-blur-lg shadow-2xl rounded-2xl border border-primary/20 p-4 flex items-center justify-between">
                <div className="flex items-center text-primary-foreground font-medium">
                  ...
                  <span>
                    <span className="font-bold">{selectedAccounts.length}</span> account
...
export default GeneralAccountsTable; 