...
import { useGeneralAccountManagement } from "@/hooks/useGeneralAccountManagement";
import { DEFAULT_GENERAL_ACCOUNT_SEARCH_FIELDS } from "@/components/table/constants/filters";

export function GeneralAccountsTable() {
  const {
    // Data
    ...
    clearAllFilters,
  } = useGeneralAccountManagement();

  // Handle account actions (disabled)
  ...
  const showFilter = hasActiveFilters || (filteredAccounts && filteredAccounts.length > 0);
          ...
          <div className="relative">
            <Select value={searchField} onValueChange={setSearchField}>
              <SelectTrigger className="w-[140px] h-12 bg-background/60 backdrop-blur-md text-foreground border border-primary/20 hover:border-primary/40 focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all duration-300 hover:bg-background/80 shadow-lg rounded-xl">
                <SelectValue>
                  {DEFAULT_GENERAL_ACCOUNT_SEARCH_FIELDS.find(
              ...
              <SelectContent className="bg-background/95 backdrop-blur-xl text-foreground border border-primary/20 rounded-xl shadow-2xl">
                {DEFAULT_GENERAL_ACCOUNT_SEARCH_FIELDS.map((opt) => (
                  <SelectItem
                    key={opt.id}
                    ...
                    <div>
                      ...
                      <Select value={searchField} onValueChange={setSearchField}>
                        ...
                        <SelectContent className="bg-background/95 backdrop-blur-md border-primary/20">
                          {DEFAULT_GENERAL_ACCOUNT_SEARCH_FIELDS.map((option) => (
      ...
      {/* <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Badge variant="secondary" className="bg-primary/10 text-primary border-primary/20">
            ...
            {filteredAccounts?.length || 0} accounts
          </Badge>
          {searchQuery && (
