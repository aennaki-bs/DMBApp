...
interface CircuitManagementTableProps {
  onRefetchReady?: (refetchFn: () => void) => void;
}

export function CircuitManagementTable({
  onRefetchReady,
}: CircuitManagementTableProps) {
  const [searchQuery, setSearchQuery] = useState("");
  const [searchField, setSearchField] = useState("all");
  const [statusFilter, setStatusFilter] = useState("any");
  const [editingCircuit, setEditingCircuit] = useState<any | null>(null);
  const [deletingCircuit, setDeletingCircuit] = useState<any | null>(null);
  ...
  useEffect(() => {
    if (onRefetchReady && refreshCircuits) {
      onRefetchReady(refreshCircuits);
    }
  }, [onRefetchReady, refreshCircuits]);
  ...
  // Handle edit circuit submission
  const handleEditSubmit = async (circuit: any) => {
    try {
      await handleEditCircuit(circuit);
      setEditingCircuit(null);
    } catch (error) {
      console.error("Failed to edit circuit:", error);
    }
  };
  ...
  const handleDeleteSubmit = async (circuit: any) => {
    try {
      await handleDeleteCircuit(circuit);
    ...
    >
      ...
      <div className="flex-1 min-h-0">
        <CircuitTableContent
          ...
          onSelectCircuit={(circuitId: number) => {
            const isSelected = selectedCircuits.includes(circuitId);
            handleSelectCircuit(circuitId, !isSelected);
          }}
      ...
      {editingCircuit && (
        <EditCircuitDialog
          ...
          onSuccess={() => {
            handleEditSubmit(editingCircuit);
            refreshCircuits();
          }}
        />
      )}
      ...
      {deletingCircuit && (
        <DeleteConfirmDialog
          ...
          onConfirm={() => handleDeleteSubmit(deletingCircuit)}
          confirmText="Delete"
          destructive={true}
        />
      )}
