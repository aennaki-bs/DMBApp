...
const subTypeService = {
  ...
  // Check if a series/subtype is used by any documents
  checkSeriesUsage: async (seriesId: number): Promise<{ isUsed: boolean; documentCount: number }> => {
    try {
      // Try to check if there's a specific API endpoint for this
      // If not available, we'll fetch documents and check manually
      try {
        const response = await api.get(`/Series/${seriesId}/has-documents`);
        return {
          isUsed: response.data,
          documentCount: response.data ? 1 : 0 // Exact count not available from this endpoint
        };
      } catch (error: any) {
        // If the endpoint doesn't exist (404), fall back to manual check
        if (error.response?.status === 404) {
          console.log('Series usage endpoint not available, checking documents manually');

          // Fetch all documents and check if any use this series
          const documentsResponse = await api.get('/Documents');
          const documents = documentsResponse.data || [];
          const documentsWithSeries = documents.filter((doc: any) => doc.subTypeId === seriesId);

          return {
            isUsed: documentsWithSeries.length > 0,
            documentCount: documentsWithSeries.length
          };
        }
        throw error;
      }
    } catch (error) {
      console.error(`Error checking series usage for ID ${seriesId}:`, error);
      // If we can't determine usage, assume it's safe to edit (not in use)
      // This prevents blocking user actions when the check fails
      return { isUsed: false, documentCount: 0 };
    }
  },

  // Check usage for multiple series at once (more efficient)
  checkMultipleSeriesUsage: async (seriesIds: number[]): Promise<Record<number, { isUsed: boolean; documentCount: number }>> => {
    try {
      // Check if we can get all documents at once and then filter
      const documentsResponse = await api.get('/Documents');
      const documents = documentsResponse.data || [];

      // Create a usage map for all series
      const usageMap: Record<number, { isUsed: boolean; documentCount: number }> = {};

      // Initialize all series as not used
      seriesIds.forEach(id => {
        usageMap[id] = { isUsed: false, documentCount: 0 };
      });

      // Count documents for each series
      documents.forEach((doc: any) => {
        if (doc.subTypeId && seriesIds.includes(doc.subTypeId)) {
          if (!usageMap[doc.subTypeId]) {
            usageMap[doc.subTypeId] = { isUsed: false, documentCount: 0 };
          }
          usageMap[doc.subTypeId].documentCount++;
          usageMap[doc.subTypeId].isUsed = true;
        }
      });

      return usageMap;
    } catch (error) {
      console.error('Error checking multiple series usage:', error);
      // If we can't determine usage, assume all series are safe to edit
      const safeUsageMap: Record<number, { isUsed: boolean; documentCount: number }> = {};
      seriesIds.forEach(id => {
        safeUsageMap[id] = { isUsed: false, documentCount: 0 };
      });
      return safeUsageMap;
    }
  }
