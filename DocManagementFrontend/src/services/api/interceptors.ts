  ...
  api.interceptors.request.use(
    async (config) => {
      ...
      if (config.method === 'post' || config.method === 'put' || config.method === 'delete') {
        ...
        // For sensitive operations like login, don't show the notification
        if (!config.url?.includes('/Auth/login') && !config.url?.includes('/Auth/register')) {
          notifications.loading('Processing request...', { id: requestId });
        }
      }
  ...
  api.interceptors.response.use(
    (response) => {
      ...
      if (response.config.requestId) {
        // @ts-ignore
        notifications.dismiss(response.config.requestId);
      }
    ...
    async (error) => {
      ...
      if (error.config?.requestId) {
        // @ts-ignore
        notifications.dismiss(error.config.requestId);
      }
      ...
      if (error.message?.includes('SSL') || error.code === 'ERR_SSL_PROTOCOL_ERROR') {
        ...
        if (!error.config?.url?.includes('/Auth/login') && !error.config?.url?.includes('/Auth/register')) {
          notifications.error('SSL connection error. Contact your administrator to configure correct API settings.');
        }
